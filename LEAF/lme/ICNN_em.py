"""Energy-management ICNN utilities trained with value and gradient supervision."""
...
def icnn_forward(params: Params, x: jnp.ndarray) -> jnp.ndarray:
    ...
    return jnp.dot(act_p(params["v"]), z) + jnp.dot(params["a"], x) + params["c"]
...
def _as_training_arrays(
    ...
    if gj.shape != Xj.shape:
        raise ValueError(f"g must have shape {Xj.shape}")
